...
class BaileysTransport:
    ...
    """
    def __init__(self, bridge_url: str, business_slug: str, observe_only: bool = False):
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(
                    ...
                    json={"business_slug": self.business_slug, "to": to, "text": text},
                    headers={"X-Internal-Secret": settings.internal_scheduler_secret or ""},
                )
                return resp.status_code == 200
        except Exception as e:
            logger.error("baileys_send_failed", error=str(e), business=self.business_slug)
            return False
    ...
    async def get_session_status(self) -> str:
        try:
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                resp = await client.get(f"{bridge_url}/qr/{self.business_slug}")
                data = resp.json()
...
def get_whatsapp_transport(prop) -> "BaileysTransport | None":
    """Factory: returns the appropriate transport for a business."""
