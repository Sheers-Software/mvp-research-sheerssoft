...
export default function PortalBusinessesPage() {
    const { user } = useAuth();
    const { tenantId, businesses, loading, refresh } = useTenant();
    const [showAddForm, setShowAddForm] = useState(false);
    const [addForm, setAddForm] = useState({ business_name: '', timezone: 'Asia/Kuala_Lumpur' });
    ...
    const handleAdd = async () => {
        if (!addForm.business_name.trim() || !tenantId) return;
        setSaving(true);
        setError('');
        setSuccess('');
        try {
            await apiPost(`/onboarding/add-business/${tenantId}`, addForm);
            setSuccess(`Business "${addForm.business_name}" created successfully`);
            setAddForm({ business_name: '', timezone: 'Asia/Kuala_Lumpur' });
            setShowAddForm(false);
            refresh();
        } catch (e: unknown) {
            setError((e instanceof Error ? e.message : String(e)) || 'Failed to add business');
        } finally {
            setSaving(false);
        }
    };
        ...
        <div>
            ...
            {showAddForm && (
                <div className="card animate-in" style={{ marginBottom: 20, borderColor: 'rgba(99,102,241,0.3)', padding: 24 }}>
                    <h3 style={{ marginBottom: 16, fontSize: 14 }}>Add New Business</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                        <div>
                            <label className="text-sm" style={{ display: 'block', marginBottom: 4 }}>Business Name</label>
                            <input
                                type="text"
                                value={addForm.business_name}
                                onChange={(e) => setAddForm((f) => ({ ...f, business_name: e.target.value }))}
                                placeholder="e.g. Sarah Photography Studios"
                    ...
                    <div className="flex items-center gap-sm">
                        ...
                        <button
                            ...
                            onClick={() => { setShowAddForm(false); setAddForm({ business_name: '', timezone: 'Asia/Kuala_Lumpur' }); }}
                        >
                            Cancel
                        </button>
                    </div>
